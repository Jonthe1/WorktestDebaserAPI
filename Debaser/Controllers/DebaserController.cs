    ...
    public class DebaserController : Controller
    {
        ...
        private IEnumerable<DebaserData> GetData()
        {
            using (WebClient wc = new WebClient())
            {
                ...
                var jsonString = wc.DownloadString("http://debaser.se/debaser/api/?version=2&method=getevents&venue=medis&from=20130101&to=20140201&format=json");
                byte[] bytes = Encoding.Default.GetBytes(jsonString);

                var newJsonString = jsonString = Encoding.UTF8.GetString(bytes);

                var debaserData1 = JsonConvert.DeserializeObject<IEnumerable<DebaserData>>(newJsonString);
                return debaserData1;
            }
